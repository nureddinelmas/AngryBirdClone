...
//  AngryBirdClone
//
//  Created by Nureddin Elmas on 2022-11-01.
//
   ...
   // var bird2 = SKSpriteNode()
            ...
            if let touch = touches.first {
                ...
                if touchNodes.isEmpty == false {
                    for node in touchNodes {
                        ...
                        if let sprite = node as? SKSpriteNode {
                            if sprite == bird {
                               let dx =  -2 * (touchLocation.x - originalPosition!.x)
                                let dy = -2 * (touchLocation.y - originalPosition!.y)
    ...
    override func update(_ currentTime: TimeInterval) {
        if let birdPhysicsBody = bird.physicsBody {
            ...
            if birdPhysicsBody.velocity.dx <= 0.15 && birdPhysicsBody.velocity.dy <= 0.15 && birdPhysicsBody.angularVelocity <= 0.15 && gameStarted == true {
